 ...
 * @license http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class message_sms_event_observers_task_testcase extends advanced_testcase {
    ...
    public function test_message_viewed_observer() {
        ...
        $this->assertEquals(2, $DB->count_records('message_sms_messages'));
        ...
        $smsmessage = $DB->get_records('message_sms_messages');

        // Check there is now only 1 message pending to be sent in the digest and it is the correct message.
        $this->assertEquals(1, count($smsmessage));

        $smsmessage = reset($smsmessage);

        $this->assertEquals($user2->id, $smsmessage->useridto);
        $this->assertEquals($conversation->id, $smsmessage->conversationid);
        $this->assertEquals($messageid2, $smsmessage->messageid);
    }
}
