...
/**
 * Contains the definiton of the sms message processors (sends messages to users via sms)
 *
 * @package   message_sms
...
class message_output_sms extends message_output {
    /**
     * Processes the message (sends by sms).
	...
	$recipient = clone($eventdata->userto);
	$recipient->email = $eventdata->userto->phone2."@meksms.mekorot.co.il";
        ...
        // We sms messages from private conversations straight away, but for group we add them to a table to be sent later.
        $smsuser = true;
        ...
        if ($smsuser) {
            ...
            $result = $DB->insert_record('message_sms_messages', $messagetosend, false);
        }
    ...
    function process_form($form, &$preferences){
        ...
        if (isset($form->sms_sms)) {
            $preferences['message_processor_sms_sms'] = clean_param($form->sms_sms, PARAM_sms);
        }
    ...
    function load_data(&$preferences, $userid){
        $preferences->sms_sms = get_user_preferences( 'message_processor_sms_sms', '', $userid);
    }
